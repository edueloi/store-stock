...
import { MapPin, Phone, Instagram, Facebook, ArrowRight, Package, ShieldCheck, Wrench, Cable } from "lucide-react";
...
export default function StoreAbout() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-12 space-y-12 bg-white">
      <StoreSEO
        title={`Sobre — ${tenant.name}`}
        description={tenant.about_text || `Conheça a ${tenant.name}: quem somos, onde estamos e como comprar material elétrico e ferragens. Atendimento via WhatsApp.`}
        url={typeof window !== "undefined" ? window.location.href : ""}
        siteName={tenant.name}
        image={tenant.banner_url || tenant.logo_url}
        keywords={`${tenant.name}, sobre, quem somos, contato, material elétrico, ${tenant.address || "loja online"}`}
        jsonLd={{
      ...
      <nav className="flex items-center gap-2 text-[10px] font-bold uppercase text-slate-400 tracking-wider">
        ...
        <span>/</span>
        <span className="text-slate-600">Sobre</span>
      </nav>

      {/* Hero */}
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div>
          <p style={{ color: style.accent }} className="text-[11px] font-black uppercase tracking-[0.4em] mb-3 flex items-center gap-2">
            <Cable size={13} /> Quem somos
          </p>
          <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none text-slate-900 mb-6">
            {tenant.name}
          </h1>
          <p className="text-slate-600 leading-relaxed text-sm font-medium">
            {tenant.about_text || "Nossa loja oferece material elétrico, ferragens e componentes com qualidade e preço justo. Priorizamos a satisfação de cada cliente, garantindo uma experiência de compra segura, ágil e prazerosa."}
          </p>
          <div className="flex flex-wrap gap-3 mt-8">
            <Link
              to={buildStorePath(slug, "/catalogo")}
              style={{ backgroundColor: style.accent }}
              className="flex items-center gap-2 px-6 h-11 rounded-xl text-white text-xs font-black uppercase tracking-widest shadow-md hover:opacity-90 transition-all"
            >
            ...
            {tenant.whatsapp && (
              <a
                ...
                className="flex items-center gap-2 px-6 h-11 rounded-xl border-2 border-slate-200 text-slate-700 text-xs font-black uppercase tracking-widest hover:bg-slate-50 hover:border-slate-300 transition-all"
              >
                Falar conosco
              </a>
            )}
        ...
        <div className={cn("aspect-square bg-slate-50 relative overflow-hidden border-2 border-slate-200", style.radius)}>
          {tenant.banner_url ? (
            <img src={tenant.banner_url} className="w-full h-full object-cover" alt="Sobre" />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
              <span className="text-slate-100 font-black text-[120px] uppercase tracking-tighter select-none">
          ...
          <div className="absolute bottom-6 left-6">
            ...
            <div className="flex items-center gap-1.5 mt-1">
              ...
              <span className="text-[11px] font-bold uppercase tracking-wider text-white/80">Loja ativa</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="p-5 border-2 border-slate-200 bg-white text-center rounded-xl shadow-sm">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="p-6 border-2 border-slate-200 bg-white space-y-5 rounded-xl shadow-sm">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Contato & Localização</p>
        ...
        <div className="p-6 border-2 border-slate-200 bg-white space-y-4 flex flex-col rounded-xl shadow-sm">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Por que comprar conosco?</p>
          {[
            { text: "Produtos com qualidade verificada", icon: <ShieldCheck size={14} /> },
            { text: "Atendimento rápido via WhatsApp", icon: <Phone size={14} /> },
            { text: "Pedido confirmado antes do pagamento", icon: <Package size={14} /> },
            { text: "Trocas e devoluções em até 7 dias", icon: <Wrench size={14} /> },
          ].map((item, i) => (
            <div key={i} className="flex items-center gap-3">
              <div style={{ backgroundColor: style.accent + "18" }} className="w-8 h-8 rounded-full flex items-center justify-center shrink-0">
