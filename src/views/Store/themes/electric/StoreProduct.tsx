...
import { ShoppingCart, Heart, Share2, ChevronLeft, ChevronRight, Package, Plus, Minus, Check, Zap, Tag, AlertCircle } from "lucide-react";
...
export default function StoreProduct() {
  ...
  if (!product) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-20 text-center">
        <Package size={48} strokeWidth={1.5} className="mx-auto mb-4 text-slate-300" />
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 space-y-10 bg-white">
      <StoreSEO
      ...
      {/* Breadcrumb */}
      <nav className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">
        <Link to={buildStorePath(slug)} className="hover:text-slate-900 transition-colors">Início</Link>
        <span className="text-slate-300">/</span>
        <Link to={buildStorePath(slug, "/catalogo")} className="hover:text-slate-900 transition-colors">Catálogo</Link>
        {category && (
          <>
            <span className="text-slate-300">/</span>
            <Link to={buildStorePath(slug, `/catalogo?cat=${category.id}`)} className="hover:text-slate-900 transition-colors">{category.name}</Link>
          </>
        )}
        <span className="text-slate-300">/</span>
        <span className="truncate max-w-[200px] text-slate-600">{product.name}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-start">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-3">
          <div className={cn("overflow-hidden relative group aspect-square bg-slate-50 border-2 border-slate-200", style.radius)}>
            {allImages.length > 0 ? (
              <img src={allImages[activeImg]} alt={product.name} className="w-full h-full object-contain p-6 transition-all duration-300" />
            ) : (
            ...
            {discountPct > 0 && (
              <span className="absolute top-4 left-4 text-white text-xs font-black px-3 py-1 rounded-full shadow-md bg-red-600">
                -{discountPct}% OFF
              </span>
            )}
            {product.is_featured && (
              <span className="absolute top-4 right-4 text-white text-[10px] font-black px-3 py-1 rounded-full flex items-center gap-1.5 shadow-md" style={{ backgroundColor: style.accent }}>
                <Zap size={11} fill="currentColor" /> Destaque
              </span>
            )}
            {allImages.length > 1 && (
              <>
                <button onClick={() => setActiveImg(i => (i - 1 + allImages.length) % allImages.length)}
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-white hover:bg-slate-50 border-2 border-slate-200 text-slate-600"
                >
                  <ChevronLeft size={16} />
                </button>
                <button onClick={() => setActiveImg(i => (i + 1) % allImages.length)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-white hover:bg-slate-50 border-2 border-slate-200 text-slate-600"
                >
                  <ChevronRight size={16} />
                </button>
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                  {allImages.map((_, i) => (
                    <button key={i} onClick={() => setActiveImg(i)} className={cn("h-1.5 rounded-full transition-all", i === activeImg ? "bg-slate-900 w-4" : "bg-slate-300 w-1.5")} />
                  ))}
                </div>
              </>
            )}
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              {allImages.map((img, i) => (
                <button key={i} onClick={() => setActiveImg(i)}
                  className={cn("w-16 h-16 shrink-0 overflow-hidden border-2 transition-all rounded-xl bg-slate-50",
                    i === activeImg
                      ? "shadow-sm scale-105"
        ...
        <motion.div
          ...
          className="flex flex-col gap-5 rounded-2xl border-2 border-slate-200 bg-white p-5 sm:p-6 md:p-8 shadow-sm"
        >
          {/* Category + Name */}
          <div>
            {category && (
            ...
            <h1 className="text-2xl md:text-3xl font-black leading-tight text-slate-900">
              {product.name}
            </h1>
            {/* SKU */}
            {product.sku && (
          ...
          {/* Price */}
          <div className="flex items-end gap-4 pb-3 border-b-2 border-slate-100">
            {product.discount_price ? (
              <>
                ...
                <div className="flex flex-col pb-1">
                  ...
                  <span className="text-[10px] font-black uppercase" style={{ color: style.accent }}>
          ...
          {/* Description */}
          {product.description && (
            <p className="text-sm text-slate-600 leading-relaxed border-t-2 border-slate-100 pt-4">
              {product.description}
            </p>
          )}
              ...
              {normalizedVariations.map((v, vi) => {
                ...
                return (
                  <div key={vi} className="space-y-2.5">
                    ...
                    {error && <p className="text-[10px] text-red-600 font-semibold">Escolha {v.name.toLowerCase()} antes de adicionar</p>}
                          ...
                          return (
                            <button key={oi} disabled={outOfStock} onClick={() => handleSelectOption(v.name, opt.value)} title={opt.value}
                              ...
                              <span className={cn("w-9 h-9 rounded-full border-2 flex items-center justify-center transition-all duration-200",
                                isSelected ? "scale-110 shadow-md border-slate-900 ring-2 ring-offset-2 ring-slate-200" : outOfStock ? "border-slate-200" : "border-transparent hover:scale-105 hover:border-slate-400")}
                                style={{ backgroundColor: bgColor }}>
                                {isSelected && <Check size={14} strokeWidth={3} className={isLight ? "text-slate-700" : "text-white"} />}
                              </span>
                              <span className={cn("text-[9px] font-semibold uppercase tracking-wide", isSelected ? "text-slate-900" : "text-slate-400")}>{opt.value}</span>
                            </button>
                          );
                        }
                        return (
                          <button key={oi} disabled={outOfStock} onClick={() => handleSelectOption(v.name, opt.value)}
                            className={cn(
                              "relative min-w-[3rem] h-11 px-4 rounded-xl border-2 text-sm font-bold transition-all duration-200 flex flex-col items-center justify-center leading-none",
                              isSelected ? "text-white shadow-md scale-105"
                                : outOfStock ? "text-slate-300 border-slate-100 bg-slate-50 cursor-not-allowed"
                                : "text-slate-600 border-slate-200 bg-white hover:border-slate-400 hover:text-slate-900 hover:scale-105"
                            )}
                            ...
                            {outOfStock && <span className="text-[8px] font-bold mt-0.5 text-red-500 leading-none">esgot.</span>}
                          </button>
                        );
                      })}
                    </div>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <div className="flex items-center h-12 overflow-hidden border-2 w-full sm:w-auto justify-between sm:justify-normal bg-white rounded-xl border-slate-200">
              <button
                onClick={() => setQty(q => Math.max(1, q - 1))}
                className="w-12 h-full flex items-center justify-center transition-all text-slate-500 hover:text-slate-900 hover:bg-slate-50"
              >
                <Minus size={14} />
              </button>
              <span className="w-10 text-center font-black text-sm text-slate-900">{qty}</span>
              <button
            ...
            <button
              ...
              className={cn(
                "flex-1 h-12 flex items-center justify-center gap-3 text-white font-black text-xs uppercase tracking-[0.14em] rounded-xl shadow-md hover:shadow-lg transition-all active:scale-95",
                minStock === 0 && "!bg-slate-100 !shadow-none !text-slate-400 cursor-not-allowed"
              )}
            >
              {justAdded ? (
          ...
          {/* Share */}
          <div className="flex items-center gap-3 pt-2 border-t-2 border-slate-100">
            <button
              onClick={handleShare}
              className="flex items-center gap-2 text-[11px] font-bold transition-colors text-slate-500 hover:text-slate-900"
            >
      ...
      {related.length > 0 && (
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-lg font-black text-slate-900">Produtos Relacionados</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {related.map(p => (
              <Link
                ...
                className="group flex flex-col border-2 transition-all overflow-hidden hover:-translate-y-1 hover:border-slate-300 hover:shadow-md bg-white border-slate-200 rounded-xl"
              >
                <div className="overflow-hidden aspect-square bg-slate-50 border-b-2 border-slate-100">
                  {p.image_url
                    ? <img src={p.image_url} alt={p.name} className="w-full h-full object-contain p-3 group-hover:scale-105 transition-transform duration-500" />
                    : <div className="w-full h-full flex items-center justify-center text-slate-300"><Package size={32} strokeWidth={1.5} /></div>}
                </div>
                <div className="p-3">
                  <p className="text-xs font-semibold text-slate-700 line-clamp-2 leading-snug">{p.name}</p>
