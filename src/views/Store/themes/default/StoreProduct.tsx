    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-3">
          <div className={cn("overflow-hidden relative group aspect-square bg-slate-100", style.radius)}>
            {allImages.length > 0 ? (
            ...
            {allImages.length > 1 && (
              <>
                <button onClick={() => setActiveImg(i => (i - 1 + allImages.length) % allImages.length)}
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 hover:bg-white"
                >
                  <ChevronLeft size={16} />
                </button>
                <button onClick={() => setActiveImg(i => (i + 1) % allImages.length)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 hover:bg-white"
                >
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <div className="flex items-center h-12 overflow-hidden border w-full sm:w-auto justify-between sm:justify-normal bg-slate-100 rounded-xl border-slate-200">
              <button
            ...
            <button
              ...
              className={cn(
                ...
                minStock === 0 && "bg-slate-200 text-slate-400 cursor-not-allowed !shadow-none"
              )}
            >
              {justAdded ? (
