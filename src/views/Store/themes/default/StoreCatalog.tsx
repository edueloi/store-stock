  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
      ...
      <nav className="flex items-center gap-2 text-[10px] font-bold uppercase mb-6 text-slate-400 tracking-wider">
        <Link to={buildStorePath(slug)} className="transition-colors hover:text-slate-700">Início</Link>
        <span>/</span>
                ...
                {[
                  ...
                  return (
                    <button
                      key={label as string}
                      onClick={() => { }}
                      className="flex items-center justify-between w-full text-[10px] transition-colors py-0.5 text-slate-500 hover:text-slate-800"
                    >
                      <span className="font-medium">{label as string}</span>
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full font-bold bg-slate-100">{count}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            <div className="flex-1 relative group w-full">
              ...
              <input
                ...
                className="w-full pl-10 pr-9 h-10 text-sm font-medium outline-none transition-all bg-white border border-slate-200 rounded-xl placeholder:text-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
              />
              {searchTerm && (
            ...
            <button
              ...
              className="lg:hidden flex items-center gap-2 h-10 px-3 text-xs font-bold border border-slate-200 bg-white rounded-xl text-slate-600"
            >
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <div ref={sortRef} className="relative">
                <button
                  ...
                  className="flex items-center gap-2 h-10 px-3 text-xs font-bold transition-all border border-slate-200 bg-white rounded-xl text-slate-600 hover:border-slate-300"
                >
                  <ArrowUpDown size={13} />
                ...
                <AnimatePresence>
                  {showSortMenu && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -6 }}
                      className="absolute right-0 top-full mt-2 rounded-2xl shadow-xl z-50 min-w-[180px] py-1 border bg-white border-slate-200"
                    >
...
function ProductCard({ product, index, slug, style, wishlist, onWishlist, onAddToCart }: CardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col overflow-hidden border transition-all duration-300 hover:shadow-xl",
        style.card,
        style.radius
      )}
    >
      ...
      <div className="flex flex-col flex-1 p-3 gap-1">
        ...
        <Link to={buildStorePath(slug, `/produto/${product.id}`)} className="text-xs font-bold text-slate-800 line-clamp-2 leading-snug hover:text-blue-600 transition-colors">
          {product.name}
        </Link>
        ...
        <div className="flex items-center justify-between mt-auto pt-2">
          ...
          <button
            ...
            style={{ backgroundColor: style.accent }}
            className={cn(
              "text-white transition-all active:scale-90 w-8 h-8 flex items-center justify-center",
              style.radius
            )}
          >
...
function ProductRow({ product, index, slug, style, wishlist, onWishlist, onAddToCart }: CardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex gap-4 border transition-all group p-3 hover:shadow-md",
        style.card,
        style.radius
      )}
    >
      <Link to={buildStorePath(slug, `/produto/${product.id}`)} className="w-24 h-24 shrink-0 rounded-xl overflow-hidden bg-slate-50 relative">
        {primaryImage
          ? <img src={primaryImage} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between">
        <div>
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{product.category_name || "Geral"}</p>
              <Link to={buildStorePath(slug, `/produto/${product.id}`)} className="text-sm font-bold text-slate-800 hover:text-blue-600 transition-colors leading-snug line-clamp-2 mt-0.5 block">
                {product.name}
              </Link>
            </div>
            <button
          ...
          {Array.isArray(product.variations) && product.variations.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {product.variations.map((v, vi) => (
                <span key={vi} className="text-[9px] font-bold px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full">
