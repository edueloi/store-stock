                  ...
                  <div key={vi} className="space-y-2.5">
                    <div className="flex items-center justify-between">
                      ...
                      {selectedOptions[v.name] ? (
                        <span className="text-[12px] font-semibold text-slate-200 bg-[#1e2d4a] px-2.5 py-0.5 rounded-full">
                          {selectedOptions[v.name]}
                        </span>
                      ) : (
                        <span className="text-[11px] text-slate-600 italic">Selecione</span>
                      )}
                    </div>
                    {error && <p className="text-[10px] text-red-400 font-semibold">Escolha {v.name.toLowerCase()} antes de adicionar</p>}
                          ...
                          return (
                            <button key={oi} disabled={outOfStock} onClick={() => handleSelectOption(v.name, opt.value)} title={opt.value}
                              ...
                              <span className={cn("w-9 h-9 rounded-full border-2 flex items-center justify-center transition-all duration-200",
                                isSelected ? "scale-110 shadow-lg border-white ring-2 ring-offset-2 ring-offset-[#071018]" : outOfStock ? "border-[#1e2d4a]" : "border-transparent hover:scale-105 hover:border-slate-400")}
                                style={{ backgroundColor: bgColor }}>
                                {isSelected && <Check size={14} strokeWidth={3} className={isLight ? "text-slate-700" : "text-white"} />}
                              </span>
                              <span className={cn("text-[9px] font-semibold uppercase tracking-wide", isSelected ? "text-slate-200" : "text-slate-500")}>{opt.value}</span>
                            </button>
                          );
                        }
                        return (
                          <button key={oi} disabled={outOfStock} onClick={() => handleSelectOption(v.name, opt.value)}
                            className={cn(
                              ...
                              isSelected ? "text-white shadow-[0_4px_16px_rgba(37,99,235,0.3)] scale-105"
                                : outOfStock ? "text-slate-600 border-[#1a2540] bg-[#0a1222]/80 cursor-not-allowed"
                                : "text-slate-300 border-[#1e2d4a] bg-[#0a1222] hover:border-blue-500/50 hover:text-white hover:scale-105"
                            )}
