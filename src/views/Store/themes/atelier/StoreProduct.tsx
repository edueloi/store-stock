...
export default function StoreProduct() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 space-y-10">
      ...
      {/* Breadcrumb */}
      <nav className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.24em] text-[#9c7b72]">
        ...
        <span>/</span>
        <span className="truncate max-w-[200px] text-slate-700">{product.name}</span>
      </nav>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-start">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-3">
          <div className={cn("overflow-hidden relative group aspect-[4/5] bg-[#f8efe8] fashion-panel border border-[#ead9ce]", style.radius)}>
            {allImages.length > 0 ? (
            ...
            {discountPct > 0 && (
              <span className="absolute top-4 left-4 text-white text-xs font-black px-3 py-1 rounded-full shadow-lg bg-red-500">
                -{discountPct}% OFF
              </span>
            )}
            {product.is_featured && (
              <span className="absolute top-4 right-4 text-white text-[10px] font-black px-3 py-1 rounded-full flex items-center gap-1.5 shadow-lg bg-amber-400">
            ...
            {allImages.length > 1 && (
              <>
                <button onClick={() => setActiveImg(i => (i - 1 + allImages.length) % allImages.length)}
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 hover:bg-white border border-[#ead9ce]"
                >
                  <ChevronLeft size={16} />
                </button>
                <button onClick={() => setActiveImg(i => (i + 1) % allImages.length)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 hover:bg-white border border-[#ead9ce]"
                >
        ...
        <motion.div
          ...
          className="flex flex-col gap-5 fashion-panel rounded-[2rem] border border-[#ead9ce] bg-white/78 p-5 sm:p-6 md:p-8"
        >
          {/* Category + Name */}
          <div>
            {category && (
              <Link to={buildStorePath(slug, `/catalogo?cat=${category.id}`)}
                className="flex items-center gap-1.5 mb-2 hover:opacity-70 transition-opacity text-[10px] font-semibold uppercase tracking-[0.24em]"
                style={{ color: style.accent }}>
                <Tag size={11} /> {category.name}
              </Link>
            )}
            <h1 className="store-display text-[2.85rem] sm:text-5xl md:text-6xl leading-[0.9] text-[#2d221f]">
              {product.name}
            </h1>
            {/* SKU */}
            {product.sku && (
          ...
          {/* Price */}
          <div className="flex items-end gap-4 pb-2 border-b border-[#eee2d6]">
            {product.discount_price ? (
              <>
                <span className="store-display text-[3.1rem] sm:text-5xl md:text-6xl leading-none text-[#2d221f]">
              ...
              <span className="store-display text-[3.1rem] sm:text-5xl md:text-6xl leading-none" style={{ color: style.accent }}>
                R$ {Number(product.price).toFixed(2)}
              </span>
            )}
          </div>
          ...
          {/* Description */}
          {product.description && (
            <p className="text-base text-[#6b5149] leading-relaxed pt-2">
              {product.description}
            </p>
          )}
          ...
          {normalizedVariations.length > 0 && (
            <div className="space-y-4 border-t border-[#ead9ce] pt-4">
                  ...
                  <div key={vi} className="space-y-2.5">
                    <div className="flex items-center justify-between">
                      ...
                      {selectedOptions[v.name] ? (
                        <span className="text-[12px] font-semibold text-[#6b5149] bg-[#f8efe8] px-2.5 py-0.5 rounded-full">
                          {selectedOptions[v.name]}
                        </span>
                      ) : (
                          ...
                          return (
                            <button key={oi} disabled={outOfStock} onClick={() => handleSelectOption(v.name, opt.value)} title={opt.value}
                              ...
                              <span className={cn("w-9 h-9 rounded-full border-2 flex items-center justify-center transition-all duration-200",
                                isSelected ? "scale-110 shadow-lg border-[#6b5149] ring-2 ring-offset-2" : outOfStock ? "border-[#ead9ce]" : "border-transparent hover:scale-105 hover:border-[#c7978d]")}
                                style={{ backgroundColor: bgColor }}>
                                {isSelected && <Check size={14} strokeWidth={3} className={isLight ? "text-slate-700" : "text-white"} />}
                              </span>
                              <span className={cn("text-[9px] font-semibold uppercase tracking-wide", isSelected ? "text-[#6b5149]" : "text-slate-400")}>{opt.value}</span>
                            </button>
                          );
                        }
                        return (
                          <button key={oi} disabled={outOfStock} onClick={() => handleSelectOption(v.name, opt.value)}
                            className={cn(
                              ...
                              isSelected ? "text-white shadow-[0_4px_16px_rgba(0,0,0,0.12)] scale-105"
                                : outOfStock ? "text-slate-300 border-slate-100 bg-slate-50/80 cursor-not-allowed"
                                : "text-[#6b5149] border-[#ead9ce] bg-white hover:border-[#c7978d] hover:scale-105"
                            )}
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <div className="flex items-center h-12 overflow-hidden border w-full sm:w-auto justify-between sm:justify-normal bg-[#f8efe8] rounded-full border-[#ead9ce]">
              <button
                onClick={() => setQty(q => Math.max(1, q - 1))}
                className="w-12 h-full flex items-center justify-center transition-all text-slate-500 hover:text-slate-900 hover:bg-slate-200"
              >
                <Minus size={14} />
              </button>
            ...
            <button
              ...
              className={cn("w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all shrink-0",
                inWishlist
          ...
          {/* Share */}
          <div className="flex items-center gap-3 pt-2 border-t border-[#eee2d6]">
            <button
              onClick={handleShare}
              className="flex items-center gap-2 text-[11px] font-bold transition-colors text-slate-400 hover:text-slate-700"
            >
      ...
      {related.length > 0 && (
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="store-display text-4xl font-semibold tracking-[-0.04em] text-[#2d221f]">Produtos Relacionados</h2>
          </div>
            ...
            {related.map(p => (
              <Link
                ...
                className={cn(
                  ...
                  style.card + " " + style.radius
                )}
              >
                <div className="overflow-hidden aspect-[4/5] bg-[#f8efe8]">
                  {p.image_url
                ...
                <div className="p-5">
                  <p className="store-display text-[1.45rem] font-semibold text-[#2d221f] line-clamp-2 leading-[0.95]">{p.name}</p>
                  <p className="store-display text-[1.7rem] font-semibold mt-3" style={{ color: style.accent }}>
