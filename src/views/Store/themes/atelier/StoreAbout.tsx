  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-12 space-y-12">
      ...
      <div className="grid md:grid-cols-2 gap-12 items-center">
        <div className="fashion-panel rounded-[2rem] border border-[#ead9ce] bg-white/78 p-5 sm:p-6 md:p-8">
          <p style={{ color: style.accent }} className="store-kicker text-[10px] font-semibold mb-3">
            Quem somos
          </p>
          <h1 className="store-display text-[3.2rem] sm:text-5xl md:text-6xl leading-[0.9] text-[#2d221f] mb-6">
            {tenant.name}
          </h1>
          <p className="text-[#6b5149] leading-relaxed text-base">
          ...
          <div className="flex flex-wrap gap-3 mt-8">
            <Link
              ...
              className="flex items-center gap-2 px-6 h-11 rounded-full text-white text-xs font-semibold uppercase tracking-[0.24em] shadow-lg hover:opacity-90 transition-all"
            >
            ...
            {tenant.whatsapp && (
              <a
                ...
                className="flex items-center gap-2 px-6 h-11 rounded-full border border-[#ead9ce] text-[#6b5149] text-xs font-semibold uppercase tracking-[0.24em] hover:bg-[#fff7f1] transition-all"
              >
                Falar conosco
              </a>
            )}
        ...
        <div className={cn("aspect-[4/5] min-h-[320px] bg-[#f1e3d8] relative overflow-hidden fashion-panel border border-[#ead9ce]", style.radius)}>
          {tenant.banner_url ? (
          ...
          <div className="absolute bottom-6 left-6">
            <p className="store-display text-4xl font-semibold text-white">{tenant.name}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className={cn("fashion-panel p-5 border text-center", style.card, style.radius)}>
            <p className="store-display text-5xl leading-none" style={{ color: style.accent }}>{s.value}</p>
            <p className="text-[10px] font-semibold text-[#8c6c63] uppercase tracking-[0.24em] mt-2">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className={cn("fashion-panel p-6 border space-y-5", style.card, style.radius)}>
          <p className="text-[10px] font-semibold text-[#8c6c63] uppercase tracking-[0.24em]">Contato & Localização</p>
        ...
        <div className={cn("fashion-panel p-6 border space-y-4 flex flex-col", style.card, style.radius)}>
          <p className="text-[10px] font-semibold text-[#8c6c63] uppercase tracking-[0.24em]">Por que comprar conosco?</p>
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium text-[#6b5149]">{item}</p>
            </div>
          ))}
          <div className="mt-auto pt-4">
            <Link
              to={buildStorePath(slug, "/catalogo")}
              style={{ backgroundColor: style.accent }}
              className="w-full h-11 flex items-center justify-center gap-2 text-white text-[11px] font-semibold uppercase tracking-[0.24em] rounded-full transition-all hover:opacity-90"
            >
