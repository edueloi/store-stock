...
export default function HeldSalesDrawer({ open, onClose, token, onResume }: HeldSalesDrawerProps) {
  ...
  async function handleResume(s: HeldSale) {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "Erro ao retomar a venda em espera.");
      load();
    } finally {
      setBusyId(null);
    }
  ...
  async function handleCancel(s: HeldSale) {
    if (!window.confirm(`Cancelar a venda em espera #${String(s.number).padStart(4, "0")}? O estoque reservado será devolvido.`)) return;
    setBusyId(s.id);
    try {
      await cancelHeldSale(token, s.id);
      setSales((prev) => prev.filter((x) => x.id !== s.id));
    } catch (err) {
      alert(err instanceof Error ? err.message : "Erro ao cancelar a venda em espera.");
    } finally {
      setBusyId(null);
    }
  }
                      ...
                      {isExpanded && (
                        <div className="px-3 pb-3 pt-0 space-y-1 bg-slate-50 border-t border-slate-100">
                          ...
                          <div className="flex gap-2 pt-2">
                            ...
                            <button onClick={() => handleCancel(s)} disabled={isBusy}
