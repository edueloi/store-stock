...
-- stock_movements: drop old FK (ignore error if not exists), add cascade
ALTER TABLE `stock_movements` DROP FOREIGN KEY `stock_movements_product_id_fkey`;
ALTER TABLE `stock_movements`
  ADD CONSTRAINT `stock_movements_product_id_fkey`
  FOREIGN KEY (`product_id`) REFERENCES `products`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- order_items: drop old FK (ignore error if not exists), add cascade
